...
process munge_gwas_vcf {
    ...
    output:
    tuple path('study_id'), path("*_harmonised_sumstats.vcf"), emit: expanded_gwas_vcf
...
process create_report {
  ...
  if [ -f "${study_id}_no_plots.txt" ]; then
    ...
    cp harmonisation-gwas-vcf-1-report.html multiqc_report.html
    mv harmonisation-gwas-vcf-1-report.html ${study_id}_multiqc_report.html
  fi
  """
}

workflow lifebitai_harmonisation_gwas_vcf{
    take:
        ch_sumstats
    ...
    main:
        ...
        if (params.force_munge) {
            munge_gwas_vcf(ch_sumstats
                            ch_field_descriptions,
                            ch_snplocs_grch37,
                            ch_snplocs_grch38)
            
            ch_expanded_gwas_vcf = munge_gwas_vcf.out.expanded_gwas_vcf
        }

        if (!params.force_munge) {
            expand_gwas_vcf(ch_sumstats)
            ch_expanded_gwas_vcf = expand_gwas_vcf.out.expanded_gwas_vcf
        }
...
workflow {
    ...
    if (params.input_type == 'list' ) {
        ch_sumstats = Channel.fromPath(params.input)
            ...
            .map { vcf -> file(vcf) }
            .take(params.take_n_studies) //default is -1 i.e. take all files (but param is useful for testing with fewer files)
    } else if (params.input_type == 'single') {
        ch_sumstats = Channel.fromPath(params.input)
    }

    lifebitai_harmonisation_gwas_vcf(ch_sumstats)
}
